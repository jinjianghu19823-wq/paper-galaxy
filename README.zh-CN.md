...
当前状态：public alpha / Phase 7 专业化阶段。仓库已经可以扫描本地示例语料，导出自包含的离线 `galaxy.html`，把文档和文本块索引到本地 SQLite，增量重建索引，用 SQLite FTS5 搜索本地文本，并启动一个本地英文/简体中文浏览器应用。这个本地应用包含类似 Obsidian 的动态文档图谱，可以浏览已经索引的语料。

Paper Galaxy 还会记录抽取质量报告，改进 Markdown/LaTeX/PDF 解析，支持用户显式开启的本地图像 OCR，并可选地把本地文档/文本块的 dense embeddings 存进 SQLite，用于语义搜索和邻居对比。主题簇标签由可检查的本地词项生成，可以在 SQLite 中手动重命名；文档邻居解释可以展示共享词项和匹配文本块摘录。
...
仓库现在是 public alpha。下一阶段是公开发布稳定化：收集安装反馈、抽取质量反馈、图谱可用性问题，并用不泄露隐私的方式 triage 早期 issue。未来的个人云库仍然只是设计文档，尚未实现；见 [docs/CLOUD_LIBRARY_DESIGN.zh-CN.md](docs/CLOUD_LIBRARY_DESIGN.zh-CN.md)。

本地应用仍然没有云依赖、Zotero 集成、桌面打包、账号系统、遥测、LLM 聊天、强制 LLM 标签、远程插件加载，或 React/Node 前端工具链。
