...
当前状态：Phase 7 专业化阶段。仓库已经可以扫描本地示例语料，导出自包含的离线 `galaxy.html`，把文档和文本块索引到本地 SQLite，增量重建索引，用 SQLite FTS5 搜索本地文本，并启动一个本地英文/简体中文浏览器应用。这个本地应用包含类似 Obsidian 的动态文档图谱，可以浏览已经索引的语料。
...
仓库已经公开，并包含静态 GitHub Pages 演示与公开发布检查。未来的个人云库仍然只是设计文档，尚未实现；见 [docs/CLOUD_LIBRARY_DESIGN.zh-CN.md](docs/CLOUD_LIBRARY_DESIGN.zh-CN.md)。
