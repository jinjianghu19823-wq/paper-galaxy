...
Paper Galaxy 目前有两种主要使用方式：

1. 从本机 Zotero Desktop 导入文献库或某个 collection，生成本地 Reading Graph。
2. 不用 Zotero，直接索引你自己的本地论文、笔记和草稿文件夹。

两种方式都会把项目状态写入 `.paper-galaxy/`，然后从你自己的电脑启动本地网页应用。Paper Galaxy 不会上传文档，也不需要托管后端。
...
### 方式 A：从 Zotero Desktop 导入

先打开 Zotero Desktop，让 Zotero 的本地 API 可用。Paper Galaxy 通过
`http://localhost:23119/api/` 读取 Zotero 本地库，并且对 Zotero 只读。
...
如果只想导入一个 collection，可以传 Zotero collection 的名称、路径或 key：
...
- `--collection "名称"`：导入某个 collection。
- `--tag "标签名"`：导入带有某个标签的条目。
- `--item-type journalArticle`：按 Zotero 条目类型筛选。
...
### 方式 B：不用 Zotero，直接导入本地文件夹

可以。Paper Galaxy 目前已经能直接索引本地论文和笔记文件夹。也就是说，它不依赖 Zotero；如果你的 paper 都在某个目录里，可以直接导入这个目录。
...
源文件夹可以放在 Paper Galaxy 仓库之外。默认情况下，Paper Galaxy 不复制或移动你的源文件；它只把抽取文本、文本块、元数据、标签、可选向量和地图运行结果写入 `.paper-galaxy/`。

也可以先用仓库自带的合成示例语料做一次 smoke test：
...
- `.markdown`
- `.tex`，使用保守的结构抽取
- `.pdf`，需要安装 `pdf` extra 中的 `pypdf`
- `.png`、`.jpg`、`.webp`、`.tiff` 等图片，但只有在安装本地 OCR 依赖并显式使用 `--include-images --ocr` 时才会处理

如果你只是想生成一个单文件离线 HTML，而不是使用 SQLite 支撑的本地网页应用，可以运行：

```bash
paper-galaxy scan /path/to/your/papers --out galaxy.html --force
```
