...
Schema v10 扩展了替代隐式 `CREATE IF NOT EXISTS` 的显式、只向前生命周期：

- 新数据库在一个明确事务中直接 bootstrap 到当前版本，并在 `schema_migrations` 记录当前 migration registry；函数返回前自行 commit。
- 当前最早支持的历史版本是 v6。测试 fixture 来自仓库真实历史，升级必须按 registry 执行 v6 -> v7 -> v8 -> v9 -> v10。v7 新增 migration history、结构化 run error 与 Zotero child-version manifest；v8 增加 document/chunk/vector provenance；v9 增加路径私有的 corpus/Zotero source registry、持久 job 状态机和历史 profile 回填；v10 增加带 CAS revision 的 registered-profile cursor、类型化 sync run detail、私有 child cache、带版本的 profile-item membership、已确认删除 tombstone 与 soft-delete state。从 v9 迁移的 profile 不猜测旧全局 cursor，而是建立一次重新 materialize 共享 document 的完整基线。v9 -> v10 回归测试使用真实发布过的冻结 v9 schema fixture，而不是给当前 schema 改版本号。
...
- 低于 v6 且没有受支持路径的数据库会被明确拒绝；高于当前版本的 future schema 同样会被拒绝，绝不会降级或把版本号改回 v10。
...
Embedding inference 与 vector encoding 同样发生在写事务之外，验证后的 vectors 按有界 batch 提交；每批落库前再次比较 document/chunk source revision。文档或 chunks 被替换/停用时会删除对应 vectors；完全相同的 Zotero sync 会跳过 PDF 抽取并保留 chunks/vectors。Zotero 网络获取、规范化和 PDF 抽取不会包在长写事务内：规范 locator/profile identity、source 行、sync state 与 running audit 会在 fetch 前由同一个初始事务登记，条目以短事务提交，精确 registered profile 的 cursor 只在全部成功后的最后事务推进。`/items?since=` 同批返回 parent/child 变化，`/deleted?since=` 只有在全部分页与 endpoint 共享同一个 `Last-Modified-Version` 时才 reconcile。child-only 更新使用私有 cache 与有界 parent hydration，不再逐 parent 请求 children。Deletion-feed tombstone 可以从 parent 移除 child；删除 parent 会级联停用缓存 child、attachment、所有 profile membership、关联 document 与 vectors。collection 单独重命名或删除时，会在 cursor 发布前补取并重建受影响的 parent；没有 deletion evidence 的遗漏仍是错误。

filter cursor 按 profile 隔离，但规范化 Zotero item 与 Paper Galaxy document 按 source 共享。source-global materialization signature 覆盖 attachment root、include flags、PDF policy、阅读状态标签集合、最小文本长度和 chunk 配置；改变它必须显式使用 `--full`，并让同 source 的其他 profile membership/cursor 等待新 baseline。持久 job 会继承最近一次兼容的 completed materialization 配置。`zotero_profile_items` 保存 `is_member` 和 `observed_version`；changed parent 会按每个 compatible active profile 的持久 filter 更新 membership，但不借用或推进其他 cursor。document activity 取所有未移除 profile membership 的并集；因此离开一个 filter 或移除 source 时，只停用没有被其他 active profile 覆盖的 document。Locator preflight 本身不写库；项目建立规范 loopback origin 与可选 data directory 后，mismatch 会在 connector 或 writer 启动前被拒绝。初始登记的 `BEGIN IMMEDIATE` 会再次核对 source row 与全部 active profile 的精确 locator，因此两个陈旧并发 preflight 不能覆盖已经建立的 data directory。首次完整远端响应前失败的 locator claim 会退出 active 使用，但保留 failed run 与 removed profile audit。source-wide 已发布 Zotero version 是单调 fence，会在每个业务事务、in-flight commit guard 后和最终 cursor 发布时重查，防止旧 profile snapshot 混入较新的 library generation。只有完整远端响应通过统一 version 校验与 source-wide fence 后，full sync 才准备新 materialization generation；stale full response 不会使 peer membership 或当前已发布 documents、chunks、vectors 失效。改变内容配置的 full sync 会先在写锁外完成 attachment resolution、PDF 读取、文本组装与 chunking，再在一个事务中提交 generation、membership、documents、chunks、vectors、cursor 与 run completion；取消、失败或进程崩溃不会暴露半代数据。Cursor CAS、run completion、profile success 与 job ownership fencing 位于同一个最终事务。
